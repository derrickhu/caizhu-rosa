/**
 * 微信小游戏真机 Pixi 适配（参考 game2D_huahua/docs/PixiJS微信小游戏真机适配踩坑记录.md）
 * 必须在任意 `new PIXI.Application()` 之前执行 → 由 main.ts 最先 import。
 */
  ...
  try {
    ...
    try {
      if (typeof _api.createOffscreenCanvas === 'function') {
        const test = _api.createOffscreenCanvas({ type: '2d', width: 1, height: 1 });
        const testCtx = test.getContext('2d');
        if (testCtx) _useOffscreen = true;
      }
    } catch { /* 降级 */ }
    ...
    settings.ADAPTER = {
      ...
      getNavigator: (): any => ({ userAgent: 'wxgame', gpu: null }),
      getBaseUrl: (): string => '',
...
if (!(ShaderSystem.prototype as any).__patched) {
  ...
  function patchedSyncUniforms(group: any, uniformData: any, ud: any, uv: any, renderer: any): void {
    let textureCount = 0; let v: any = null; let cv: any = null;
    const gl = renderer.gl;
    for (const i in group.uniforms) {
      const data = uniformData[i]; const uvi = uv[i]; const udi = ud[i]; const gu = group.uniforms[i];
      ...
      if (data.type === 'float' && data.size === 1 && !data.isArray) { if (uvi !== udi.value) { udi.value = uvi; gl.uniform1f(udi.location, uvi); } }
        ...
        (data.size === 1 && !data.isArray ? GLSL_TO_SINGLE_SETTERS : GLSL_TO_ARRAY_SETTERS)[data.type]
          .call(null, gl, udi.location, udi.value, uvi);
      }
    }
  }
      ...
      try {
        ...
        if (_canReadPixels
          ...
          try {
            ...
            if (_uploadLog < 6) {
              console.warn('[pixiWechatPatch] canvas 覆盖失败', e);
              _uploadLog++;
            }
          }
        }
