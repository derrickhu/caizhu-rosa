...
class TweenManagerClass {
  ...
  update(dt: number): void {
    ...
    for (const tween of this._tweens) {
      ...
      if (tgt == null || (typeof tgt.destroyed === 'boolean' && tgt.destroyed)) {
        aborted.push(tween); continue;
      }
      ...
      if (config.onUpdate) {
        try { config.onUpdate(); } catch { aborted.push(tween); continue; }
      }
    ...
    for (const t of aborted) {
      const idx = this._tweens.indexOf(t);
      if (idx !== -1) this._tweens.splice(idx, 1);
    }
    for (const t of completed) {
      const idx = this._tweens.indexOf(t);
      if (idx !== -1) this._tweens.splice(idx, 1);
      if (t.config.onComplete) t.config.onComplete();
    }
  }
