...
class CloudSyncManagerClass {
  ...
  async awaitAuthoritativeStartup(): Promise<CloudStartupSyncResult> {
    if (!this.enabled) {
      this.authorityState = 'disabled';
      return { status: 'disabled', reason: 'backend-disabled' };
    }
    ...
    try {
      ...
      // Initialization handles its own fallback state.
    }
    if (this.authorityState === 'cacheOnly') {
      return { status: 'cache-only', reason: 'startup-failed' };
    }
    return {
      status: this.lastStartupRemoteApplied ? 'remote-applied' : 'confirmed',
      reason: this.lastStartupRemoteApplied ? 'remote-imported' : 'cloud-confirmed',
  ...
  private async pullFromCloudOnStartup(): Promise<void> {
    ...
    if (!remote.exists || Object.keys(remote.payload || {}).length === 0) {
      ...
      if (hasLocalData) {
        ...
        this.scheduleSync('startup-local-to-empty-remote');
      } else {
    ...
    if (shouldApplyRemote) {
      ...
      if (hasLocalData) {
        PersistService.touchCloudMeta();
        this.scheduleSync('startup-merged-local-remote');
      }
      return;
    }
  ...
  private async syncToCloud(reason: string, force = false): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        if (error instanceof BackendError && error.code === 'STALE_UPDATE' && error.data?.remote) {
          const remote = error.data.remote as { updatedAt?: number; payload?: Record<string, string> };
          PersistService.importCloudSnapshot({
          ...
          PersistService.touchCloudMeta();
          this.scheduleSync('stale-update-merged');
          this.syncFailCount = 0;
          this.syncDisabled = false;
          return;
        }
      ...
      if (this.syncFailCount <= CLOUD_SYNC_LOG_THRESHOLD) {
        console.warn(`[CloudSync] push failed (${this.syncFailCount}/${CLOUD_SYNC_MAX_FAIL_COUNT})`, error?.message || error);
      }
